...
class PlacesManager {
	...
	* @param $id
	*/
	public function getPlace($id, $isEagerFetch = false) {
	if ($id) {
		...
		if ($isEagerFetch) {
			$country_manager = new CountriesManager($this->bdd);
	...
	*/
	...
	public function getPlaces($country_id = 0, $isEagerFetch = false) {
		$places = array();
		if ($country_id > 0) {
			$q = $this->bdd->prepare('SELECT * FROM places WHERE country_id = :country_id ORDER BY name');
			$q->bindValue(':country_id', $country_id, PDO::PARAM_INT);
			}
		else {
			$q = $this->bdd->prepare('SELECT * FROM places ORDER BY name');
		}
		$q->execute();
		while ($data = $q->fetch(PDO::FETCH_ASSOC)) {
	...
	public function getPlacesByPage($country_id, $page_num, $lpp, $isEagerFetch = false) {
		$places = array();
	 ...
	 */
	public function getPlacesForSelect($country_id) {
		$places = array();
		$q = $this->bdd->prepare('SELECT p.id, p.name, c.name AS country_name FROM places p 
								  JOIN countries c ON c.id = p.country_id WHERE country_id = :country_id ORDER BY c.id');
		$q->bindValue(':country_id', $country_id, PDO::PARAM_INT);
		$q->execute();
